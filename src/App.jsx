...
const C = {
  bg: "#ffffff", bgOff: "#f8f8f8", bgInput: "#f5f5f5",
  border: "#e8e8e8", borderDark: "#222", text: "#111111",
  textMid: "#555555", textLight: "#999999", accent: "#111111",
  accentLight: "#f0f0f0", danger: "#cc3333", dangerLight: "#fff0f0", rec: "#cc3333", line: "#ebebeb",
};
...
// ── Logo — field scene fills full circle, no wrapper needed ───────────────
const Logo = ({ size = 36 }) => (
  <svg width={size} height={size} viewBox="0 0 60 60">
    <clipPath id="logoClip"><circle cx="30" cy="30" r="30"/></clipPath>
    <g clipPath="url(#logoClip)">
      <rect x="0" y="0" width="60" height="60" fill="#cce8f7"/>
      <circle cx="48" cy="12" r="7" fill="#f7d44c"/>
      <rect x="0" y="34" width="60" height="26" fill="#7aaa87"/>
      <rect x="0" y="40" width="60" height="20" fill="#6a9a77"/>
      <path d="M2 42 Q30 36 58 42" stroke="#7ab8d4" strokeWidth="5" fill="none" strokeLinecap="round"/>
      <path d="M2 42 Q30 36 58 42" stroke="#9ed0e8" strokeWidth="2.5" fill="none" strokeLinecap="round" opacity="0.5"/>
      <ellipse cx="5" cy="34" rx="7" ry="9" fill="#5a8a46"/>
      <ellipse cx="55" cy="35" rx="6" ry="8" fill="#5a8a46"/>
      <path d="M14 26 Q17 22 20 26" stroke="#4a6a8a" strokeWidth="1.2" fill="none" strokeLinecap="round"/>
      <path d="M22 22 Q25 18 28 22" stroke="#4a6a8a" strokeWidth="1.2" fill="none" strokeLinecap="round"/>
      <ellipse cx="26" cy="45" rx="4" ry="2.5" fill="#c4a070"/>
      <circle cx="26" cy="42" r="3" fill="#c4a070"/>
      <ellipse cx="38" cy="44" rx="3" ry="2" fill="#a08060"/>
    </g>
  </svg>
);

// ── Logo on dark background version ───────────────────────────────────────
const LogoDark = ({ size = 36 }) => (
  <svg width={size} height={size} viewBox="0 0 60 60">
    <clipPath id="logoDarkClip"><circle cx="30" cy="30" r="30"/></clipPath>
    <g clipPath="url(#logoDarkClip)">
      <rect x="0" y="0" width="60" height="60" fill="#cce8f7"/>
      <circle cx="48" cy="12" r="7" fill="#f7d44c"/>
      <rect x="0" y="34" width="60" height="26" fill="#7aaa87"/>
      <rect x="0" y="40" width="60" height="20" fill="#6a9a77"/>
...
function MoodPicker({ value = [], onChange, customMoods = [], onAddCustomMood }) {
  ...
  const moodColor = (m) => {
    const pinks = ["✨ hopeful","🌊 overwhelmed","🌧 heavy","😶 numb"];
    return pinks.includes(m) ? "#e8a0a8" : "#8fba9a";
  };
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
        {allMoods.map(m => (
          <button key={m} onClick={() => toggle(m)} style={{
            background: value.includes(m) ? moodColor(m) : C.bgInput,
...
function AuthScreen() {
  ...
  const inputStyle = { width: "100%", background: C.bgInput, border: `1px solid ${C.border}`, borderRadius: 6, padding: "12px 14px", color: C.text, fontSize: 14, fontFamily: "inherit", outline: "none", boxSizing: "border-box", transition: "border-color 0.15s" };
  const btnPrimary = { width: "100%", padding: "13px", borderRadius: 6, background: C.text, border: "none", color: "#fff", fontSize: 13, cursor: "pointer", fontFamily: "inherit", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 };
    ...
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ width: "100%", maxWidth: 360, animation: "slideUp 0.45s ease both" }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          <Logo size={72} />
          <h1 style={{ margin: "14px 0 0", fontSize: 38, fontWeight: 400, fontFamily: "Georgia, serif", color: C.text, letterSpacing: "-0.02em" }}>Momentz</h1>
        ...
        <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 12, padding: 24 }}>
          {mode === "login" && (<>
            <div style={{ marginBottom: 14 }}>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@email.com" style={inputStyle} onFocus={e => e.target.style.borderColor = C.text} onBlur={e => e.target.style.borderColor = C.border} />
            </div>
            <div style={{ marginBottom: 16 }}>
              ...
              <input type="password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === "Enter" && handleLogin()} placeholder="••••••••" style={inputStyle} onFocus={e => e.target.style.borderColor = C.text} onBlur={e => e.target.style.borderColor = C.border} />
            </div>
            ...
            <div style={{ marginTop: 16, display: "flex", justifyContent: "center", gap: 20 }}>
              <button onClick={() => { setMode("signup"); setError(""); }} style={{ background: "none", border: "none", color: C.textMid, fontSize: 12, cursor: "pointer", fontFamily: "inherit" }}>Have an invite?</button>
          ...
          {mode === "signup" && (<>
            <div style={{ fontSize: 12, color: C.textMid, marginBottom: 16, textAlign: "center" }}>Enter your invite code to create an account</div>
            <div style={{ marginBottom: 14 }}>
              ...
              <input type="text" value={inviteCode} onChange={e => setInviteCode(e.target.value.toUpperCase())} placeholder="XXXXXXXX" style={{ ...inputStyle, letterSpacing: "0.25em", fontWeight: 600 }} onFocus={e => e.target.style.borderColor = C.text} onBlur={e => e.target.style.borderColor = C.border} />
            </div>
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 10, color: C.textMid, marginBottom: 6, letterSpacing: "0.12em", textTransform: "uppercase", fontWeight: 600 }}>Email</div>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@email.com" style={inputStyle} onFocus={e => e.target.style.borderColor = C.text} onBlur={e => e.target.style.borderColor = C.border} />
            ...
            <div style={{ marginBottom: 16 }}>
              <div style={{ fontSize: 10, color: C.textMid, marginBottom: 6, letterSpacing: "0.12em", textTransform: "uppercase", fontWeight: 600 }}>Password</div>
              <input type="password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSignup()} placeholder="••••••••" style={inputStyle} onFocus={e => e.target.style.borderColor = C.text} onBlur={e => e.target.style.borderColor = C.border} />
            </div>
            {error && <div style={{ fontSize: 12, color: C.danger, marginBottom: 12, padding: "8px 12px", background: C.dangerLight, borderRadius: 6 }}>{error}</div>}
          ...
          {mode === "reset" && (resetSent ? (
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@email.com" style={{ ...inputStyle, marginBottom: 12 }} onFocus={e => e.target.style.borderColor = C.text} onBlur={e => e.target.style.borderColor = C.border} />
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "0 20px 40px", backgroundImage: `repeating-linear-gradient(transparent, transparent ${lineHeight - 1}px, ${C.line} ${lineHeight - 1}px, ${C.line} ${lineHeight}px)`, backgroundSize: `100% ${lineHeight}px`, backgroundPositionY: "80px" }}>
        {moods.length > 0 && (
          <div style={{ paddingTop: 16, marginBottom: 4, display: "flex", gap: 6, flexWrap: "wrap" }}>
            {moods.map((m, i) => <div key={m} style={{ background: i % 2 === 0 ? "#e8a0a8" : "#8fba9a", color: "#fff", borderRadius: 4, padding: "3px 10px", fontSize: 10 }}>{m}</div>)}
          </div>
        )}
    ...
    <div style={{ position: "fixed", inset: 0, background: C.bg, display: "flex", flexDirection: "column", zIndex: 300 }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px 100px" }}>
          ...
          <div key={i} style={{ display: "flex", gap: 12, marginBottom: 0 }}>
            ...
            <div style={{ flex: 1, paddingBottom: 20 }}>
              ...
              <textarea ref={el => textareaRefs.current[i] = el} value={part} onChange={e => updatePart(i, e.target.value)}
                ...
                style={{ width: "100%", background: C.bgOff, border: `1px solid ${C.border}`, borderRadius: 8, padding: "12px 14px", color: C.text, fontSize: 15, fontFamily: "inherit", resize: "none", outline: "none", lineHeight: 1.65, boxSizing: "border-box" }}
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <button onClick={addPart} style={{ background: "none", border: `1px solid ${C.border}`, color: C.textMid, borderRadius: 4, padding: "8px 18px", fontSize: 11, cursor: "pointer", fontFamily: "inherit", letterSpacing: "0.1em", textTransform: "uppercase", fontWeight: 600 }}
...
function PulseCard({ thread, onDelete, user }) {
  ...
  const loadReplies = async () => {
    ...
    setLoadingReplies(false); setExpanded(true);
  };
  const saveReply = async () => {
    ...
    try {
      ...
      if (error) throw error;
      setReplies(prev => [...prev, data]); setReplyText(""); setReplying(false);
    ...
    <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 12, marginBottom: 10, overflow: "hidden", animation: "fadeUp 0.3s ease both" }}>
      <div style={{ padding: "14px 16px 0" }}>
        ...
        {!showFullThread && parts.length > 1 && (
          <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 12 }}>
            <div style={{ width: 26 }} />
            <button onClick={() => setShowFullThread(true)} style={{ background: "none", border: "none", color: C.textMid, fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>Show {parts.length - 1} more {parts.length - 1 === 1 ? "part" : "parts"} ↓</button>
          </div>
        )}
        {showFullThread && parts.length > 1 && (
          <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 12 }}>
      ...
      <div style={{ padding: "8px 16px 12px", display: "flex", gap: 16, borderTop: `1px solid ${C.border}` }}>
        ...
        {thread.reply_count > 0 && (
          <button onClick={loadReplies} style={{ background: "none", border: "none", color: C.textLight, fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "24px 20px 52px" }}>
        ...
        {moods.length > 0 && (
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
            {moods.map((m, i) => <div key={m} style={{ background: i % 2 === 0 ? "rgba(232,160,168,0.4)" : "rgba(143,186,154,0.4)", border: `1px solid ${i % 2 === 0 ? "rgba(232,160,168,0.5)" : "rgba(143,186,154,0.5)"}`, color: "#fff", borderRadius: 4, padding: "4px 10px", fontSize: 11 }}>{m}</div>)}
          </div>
        )}
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      {view === "grid" && (
        <div style={{ maxWidth: 480, margin: "0 auto", height: "100vh", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ padding: "14px 20px 0", display: "flex", flexShrink: 0, borderBottom: `1px solid ${C.border}` }}>
            {TABS.map(t => (
            ...
            <div style={{ flex: 1, overflowY: "auto", paddingBottom: 90 }}>
              {loadingEntries ? <div style={{ padding: 70, display: "flex", justifyContent: "center" }}><Spinner /></div>
                : filteredEntries.length === 0 ? (
                  <div style={{ padding: "60px 20px", textAlign: "center" }}>
                    <Logo size={64} />
                    <div style={{ fontSize: 16, color: C.textMid, fontFamily: "Georgia, serif", fontStyle: "italic", marginTop: 16 }}>{entries.length === 0 ? "Your first moment is waiting" : "No entries for this mood"}</div>
                      ...
                      return (
                        <div key={entry.id} onClick={() => { setSwipeIndex(i); setView("swipe"); }} style={{ aspectRatio: "9/16", position: "relative", cursor: "pointer", overflow: "hidden", background: C.bgOff, animation: `fadeUp 0.3s ease ${i * 0.02}s both` }}>
                          {entry.thumbnail ? <img src={entry.thumbnail} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                            : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: "#e8e8e8" }}><Logo size={28} /></div>}
                          ...
                          {entry.duration > 0 && <div style={{ position: "absolute", top: 6, right: 6, background: "rgba(0,0,0,0.55)", borderRadius: 3, padding: "2px 5px", fontSize: 8, color: "rgba(255,255,255,0.75)" }}>{fmtDur(entry.duration)}</div>}
                          ...
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                            ...
                            {moods.length > 0 && (
                              <div style={{ display: "flex", gap: 4, flexWrap: "wrap", justifyContent: "flex-end", maxWidth: "50%" }}>
                                {moods.slice(0, 2).map((m, idx) => <div key={m} style={{ fontSize: 9, color: "#fff", background: idx % 2 === 0 ? "#e8a0a8" : "#8fba9a", borderRadius: 4, padding: "3px 8px", whiteSpace: "nowrap" }}>{m}</div>)}
            ...
            <div style={{ position: "relative", pointerEvents: "all" }}>
              {showFABMenu && tab === "video" && (
                <div style={{ position: "absolute", bottom: 72, left: "50%", transform: "translateX(-50%)", display: "flex", flexDirection: "column", gap: 8, alignItems: "center", animation: "fadeUp 0.2s ease" }}>
                  <button onClick={openRecorder} style={{ background: C.bg, border: `1px solid ${C.border}`, color: C.text, borderRadius: 6, padding: "10px 20px", fontSize: 11, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap", boxShadow: "0 4px 16px rgba(0,0,0,0.1)", display: "flex", alignItems: "center", gap: 8, letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 600 }}>⏺ Record Now</button>
                  <button onClick={openUpload} style={{ background: C.bg, border: `1px solid ${C.border}`, color: C.text, borderRadius: 6, padding: "10px 20px", fontSize: 11, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap", boxShadow: "0 4px 16px rgba(0,0,0,0.1)", display: "flex", alignItems: "center", gap: 8, letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 600 }}>↑ Upload Video</button>
                </div>
              )}
              {tab !== "pulse" && (
                <button onClick={() => { if (tab === "writing") { setShowFABMenu(false); setView("writeNew"); } else setShowFABMenu(p => !p); }}
          ...
          <div style={{ flex: 1, overflow: "hidden", position: "relative" }}>
            ...
            {uploadMode === "upload" && !previewBlob && (
              <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16, background: "#0a0a0a" }}>
                ...
                <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)" }}>Max 50MB · Compress large files first</div>
